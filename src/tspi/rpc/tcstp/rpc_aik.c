...
TSS_RESULT
TCS_GetCredentials_TP(struct host_table_entry *hte,
		      UINT32 * pcEndorsementCredentialSize,	/* out */
		      BYTE ** prgbEndorsementCredential,	/* out */
		      UINT32 * pcPlatformCredentialSize,	/* out */
		      BYTE ** prgbPlatformCredential,		/* out */
		      ...
		      BYTE ** prgbConformanceCredential)	/* out */
{
	...
	initData(&hte->comm, 1);
	hte->comm.hdr.u.ordinal = TCSD_ORD_GETCREDENTIALS;
	LogDebugFn("TCS Context: 0x%x", hte->tcsContext);
	...
	if (result == TSS_SUCCESS) {
		if (getData(TCSD_PACKET_TYPE_UINT32, 0, pcEndorsementCredentialSize, 0,
			    &hte->comm)) {
			result = TSPERR(TSS_E_INTERNAL_ERROR);
			goto done;
		}
		...
		*prgbEndorsementCredential = (BYTE *) malloc(*pcEndorsementCredentialSize);
		...
		if (getData(TCSD_PACKET_TYPE_PBYTE, 1, *prgbEndorsementCredential,
			    *pcEndorsementCredentialSize, &hte->comm)) {
			free(*prgbEndorsementCredential);
			result = TSPERR(TSS_E_INTERNAL_ERROR);
			goto done;
		}
		if (getData(TCSD_PACKET_TYPE_UINT32, 2, pcPlatformCredentialSize, 0, &hte->comm)) {
			free(*prgbEndorsementCredential);
			result = TSPERR(TSS_E_INTERNAL_ERROR);
			goto done;
		}
		...
		*prgbPlatformCredential = (BYTE *) malloc(*pcPlatformCredentialSize);
		...
		if (getData(TCSD_PACKET_TYPE_PBYTE, 3, *prgbPlatformCredential,
			    *pcPlatformCredentialSize, &hte->comm)) {
			free(*prgbEndorsementCredential);
			free(*prgbPlatformCredential);
			result = TSPERR(TSS_E_INTERNAL_ERROR);
			goto done;
		}
		if (getData(TCSD_PACKET_TYPE_UINT32, 4, pcConformanceCredentialSize, 0,
			    &hte->comm)) {
			free(*prgbEndorsementCredential);
			free(*prgbPlatformCredential);
			result = TSPERR(TSS_E_INTERNAL_ERROR);
		...
		*prgbConformanceCredential = (BYTE *) malloc(*pcConformanceCredentialSize);
		...
		if (getData(TCSD_PACKET_TYPE_PBYTE, 5, *prgbConformanceCredential,
			    *pcConformanceCredentialSize, &hte->comm)) {
			free(*prgbEndorsementCredential);
			free(*prgbPlatformCredential);
			free(*prgbConformanceCredential);
			result = TSPERR(TSS_E_INTERNAL_ERROR);
