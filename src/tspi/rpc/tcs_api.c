					     ...
					     UINT32 * keySize, BYTE ** keyBlob)
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_GetRegisteredKeyByPublicInfo_TP(entry, tcsContext, algID,
								      ulPublicInfoLength,
			   ...
			   TSS_PCR_EVENT ** ppEvent)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
		result =
			TCS_GetPcrEvent_TP(entry, hContext, PcrIndex, pNumber, ppEvent);
			break;
		default:
			break;
	}
				 ...
				 TSS_PCR_EVENT ** ppEvents)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCS_GetPcrEventsByPcr_TP(entry, hContext, PcrIndex, FirstEvent,
			      ...
			      TSS_PCR_EVENT ** ppEvents)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCS_GetPcrEventLog_TP(entry, hContext, pEventCount, ppEvents);
			break;
		default:
			break;
	}
			   ...
			   BYTE * gbVendorData)	/* in */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCS_RegisterKey_TP(entry, hContext, WrappingKeyUUID, KeyUUID,
				  ...
				  TSS_KM_KEYINFO ** ppKeyHierarchy)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCS_EnumRegisteredKeys_TP(entry, hContext, pKeyUUID,
				...
				TSS_KM_KEYINFO ** ppKeyInfo)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCS_GetRegisteredKey_TP(entry, hContext, KeyUUID, ppKeyInfo);
			break;
		default:
			break;
	}
				    ...
				    BYTE ** prgbKey)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCS_GetRegisteredKeyBlob_TP(entry, hContext, KeyUUID, pcKeySize,
							     prgbKey);
			break;
		default:
			break;
			      ...
			      TCS_KEY_HANDLE * phKeyHMAC)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_LoadKeyByBlob_TP(entry, hContext, hUnwrappingKey,
			      ...
			      TCS_KEY_HANDLE * phKeyTCSI)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_LoadKeyByUUID_TP(entry, hContext, KeyUUID, pLoadKeyInfo,
						       phKeyTCSI);
			break;
		default:
			break;
			      ...
			      TPM_AUTH * pAuth)	/* in, out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_CreateWrapKey_TP(entry, hContext, hWrappingKey, KeyUsageAuth,
			   ...
			   BYTE ** prgbPubKey)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_GetPubKey_TP(entry, hContext, hKey, pAuth, pcPubKeySize,
						   prgbPubKey);
			break;
		default:
			break;
			     ...
			     BYTE ** prgbConformanceCredential)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_MakeIdentity_TP(entry, hContext, identityAuth,
				...
				TSS_BOOL state)	/* in */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_SetOwnerInstall_TP(entry, hContext, state);
			break;
		default:
			break;
	}
			      ...
			      BYTE ** srkKey)
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_TakeOwnership_TP(entry, hContext, protocolID,
		     ...
		     TCPA_NONCE * nonce0)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_OIAP_TP(entry, hContext, authHandle, nonce0);
			break;
		default:
			break;
	}
		     ...
		     TCPA_NONCE * nonceEvenOSAP)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_OSAP_TP(entry, hContext, entityType, entityValue,
			   ...
			   BYTE ** outData)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_ChangeAuth_TP(entry, contextHandle, parentHandle, protocolID,
				...
				TPM_AUTH * ownerAuth)	/* in, out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_ChangeAuthOwner_TP(entry, hContext, protocolID, newAuth,
				    ...
				    TCS_KEY_HANDLE * ephHandle)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_ChangeAuthAsymStart_TP(entry, hContext, idHandle, antiReplay,
				     ...
				     TCPA_DIGEST * changeProof)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_ChangeAuthAsymFinish_TP(entry, hContext, parentHandle,
				...
				TCS_AUTHHANDLE handle)	/* in */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_TerminateHandle_TP(entry, hContext, handle);
			break;
		default:
			break;
	}
				    ...
				    BYTE ** SymmetricKey)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_ActivateTPMIdentity_TP(entry, hContext, idKey, blobSize, blob,
			...
			TCPA_PCRVALUE * outDigest)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_Extend_TP(entry, hContext, pcrNum, inDigest, outDigest);
			break;
		default:
			break;
	}
			...
			TCPA_PCRVALUE * outDigest)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_PcrRead_TP(entry, hContext, pcrNum, outDigest);
			break;
		default:
			break;
	}
		      ...
		      BYTE ** sig)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_Quote_TP(entry, hContext, keyHandle, antiReplay,
			     ...
			     TPM_AUTH * ownerAuth)	/* in, out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_DirWriteAuth_TP(entry, hContext, dirIndex, newContents,
						      ownerAuth);
			break;
		default:
			break;
			 ...
			 TCPA_DIRVALUE * dirValue)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_DirRead_TP(entry, hContext, dirIndex, dirValue);
			break;
		default:
			break;
	}
		     ...
		     BYTE ** SealedData)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_Seal_TP(entry, hContext, keyHandle, encAuth, pcrInfoSize,
		       ...
		       BYTE ** Data)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_Unseal_TP(entry, hContext, parentHandle, SealedDataSize,
		       ...
		       BYTE ** outData)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_UnBind_TP(entry, hContext, keyHandle, inDataSize, inData,
				    ...
				    BYTE ** outData)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_CreateMigrationBlob_TP(entry, hContext, parentHandle,
				     ...
				     BYTE ** outData)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_ConvertMigrationBlob_TP(entry, hContext, parentHandle,
				      ...
				      BYTE ** MigrationKeyAuth)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_AuthorizeMigrationKey_TP(entry, hContext, migrateScheme,
			   ...
			   BYTE ** outData)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_CertifyKey_TP(entry, hContext, certHandle, keyHandle,
		     ...
		     BYTE ** sig)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_Sign_TP(entry, hContext, keyHandle, areaToSignSize,
			  ...
			  BYTE ** randomBytes)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_GetRandom_TP(entry, hContext, bytesRequested, randomBytes);
			break;
		default:
			break;
	}
			   ...
			   BYTE * inData)	/* in */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_StirRandom_TP(entry, hContext, inDataSize, inData);
			break;
		default:
			break;
	}
			     ...
			     BYTE ** resp)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCS_GetCapability_TP(entry, hContext, capArea, subCapSize, subCap,
						      respSize, resp);
			break;
		default:
			break;
			      ...
			      BYTE ** resp)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_GetCapability_TP(entry, hContext, capArea, subCapSize, subCap,
						       respSize, resp);
			break;
		default:
			break;
				    ...
				    BYTE ** sig)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_GetCapabilitySigned_TP(entry, hContext, keyHandle, antiReplay,
				    ...
				    UINT32 * pVolatileFlags)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_GetCapabilityOwner_TP(entry, hContext, pOwnerAuth, pVersion,
					 ...
					 TCPA_DIGEST * checksum)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_CreateEndorsementKeyPair_TP(entry, hContext, antiReplay,
			  ...
			  TCPA_DIGEST * checksum)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_ReadPubek_TP(entry, hContext, antiReplay,
				 ...
				 TPM_AUTH * ownerAuth)	/* in, out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_DisablePubekRead_TP(entry, hContext, ownerAuth);
			break;
		default:
			break;
	}
			       ...
			       BYTE ** pubEndorsementKey)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_OwnerReadPubek_TP(entry, hContext, ownerAuth,
							pubEndorsementKeySize, pubEndorsementKey);
			break;
		default:
			break;
				...
				BYTE ** sig)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_CertifySelfTest_TP(entry, hContext, keyHandle, antiReplay,
			      ...
			      BYTE ** outData)	/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_GetTestResult_TP(entry, hContext, outDataSize, outData);
			break;
		default:
			break;
	}
			   ...
			   TPM_AUTH * ownerAuth)	/* in, out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_OwnerClear_TP(entry, hContext, ownerAuth);
			break;
		default:
			break;
	}
				  ...
				  TPM_AUTH * ownerAuth)	/* in, out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_DisableOwnerClear_TP(entry, hContext, ownerAuth);
			break;
		default:
			break;
	}
				       ...
				       TSS_BOOL state)	/* in */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_PhysicalSetDeactivated_TP(entry, hContext, state);
			break;
		default:
			break;
	}
				 ...
				 TCPA_PHYSICAL_PRESENCE fPhysicalPresence)	/* in */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_PhysicalPresence_TP(entry, hContext, fPhysicalPresence);
			break;
		default:
			break;
	}
...
TSS_RESULT
TCSP_DaaJoin(TCS_CONTEXT_HANDLE hContext,	/* in */
	     ...
	     BYTE** outputData)			/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_DaaJoin_TP(entry, hContext, daa_session, stage, inputSize0,
...
TSS_RESULT
TCSP_DaaSign(TCS_CONTEXT_HANDLE hContext,	/* in */
	     TPM_HANDLE daa_session,		/* in */
	     BYTE stage,			/* in */
	     UINT32 inputSize0,			/* in */
	     BYTE* inputData0,			/* in */
	     ...
	     BYTE** outputData)			/* out */
{
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = TCSP_DaaSign_TP(entry, hContext, daa_session, stage, inputSize0,
						 inputData0, inputSize1, inputData1, ownerAuth,
						 outputSize, outputData);
			break;
		default:
