...
#if 0
TSS_RESULT
send_final(struct host_table_entry *hte, BYTE *data, int dataLength, struct tcsd_packet_hdr **hdr)
{
	struct tcsd_packet_hdr loc_hdr, *hdr_p;
	int hdr_size = sizeof(struct tcsd_packet_hdr);
	int returnSize;
	...
	if (send(hte->socket, data, dataLength, 0) < 0) {
		LogError("send: %s", strerror(errno));
		result = TSS_E_COMM_FAILURE;
		goto err_exit;
	}

	if ((returnSize = recv(hte->socket, &loc_hdr, hdr_size - 1, 0)) < 0) {
		LogError("recv: %s", strerror(errno));
		result = TSS_E_COMM_FAILURE;
		goto err_exit;
	} else if (returnSize == 0) {
		...
		if (returnSize > hdr_size) {
			if ((returnSize = recv(hte->socket, &hdr_p->data, returnSize - (hdr_size-1), 0)) < 0) {
				LogError("recv: %s", strerror(errno));
				free(hdr_p);
				result = TSS_E_COMM_FAILURE;
				goto err_exit;
